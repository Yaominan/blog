...
重新安装vsftpd服务。创建用于进行FTP认证的用户数据库文件，其中奇数行为账户名，偶数行为密码。例如，分别创建zhangsan和lisi两个用户，密码均为redhat：
...
编辑配置文件/etc/auto.master和/etc/auto.misc
...
修改allow-update {192.168.1.20;}; IP为从服务器的IP，重启named
...
success
[root@minayao named]# 
在从服务器上安装bind-chroot，并修改主配置文件/etc/named.conf

listen-on port 53 { any; };
allow-query     { any; };
在从服务器中填写主服务器的IP和抓取的区域信息，重启服务
...
检验，将从服务器的dns参数改成192.168.1.20（此处如果是DHCP分配的IP会出问题，可以新建连接，用nmcli手动配置连接的参数，）并up起来
...
部署基础的电子邮件系统
配置服务器主机名，服务器主机名需要和发信域名保持一致

清空防火墙，允许DNS协议

为电子邮件系统提供域名解析（bind-chroot）
...
客户使用电子邮件新系统
配置IP在同一网段，DNS为邮件系统的IP

使用客户端(如outlook、thunderbird等)
